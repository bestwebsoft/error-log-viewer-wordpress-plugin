...
if ( ! class_exists( 'Rrrlgvwr_Settings_Tabs' ) ) {
	...
	class Rrrlgvwr_Settings_Tabs extends Bws_Settings_Tabs {
		...
		public function save_options() {
			...
			if ( ! isset( $_POST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['_wpnonce'] ) ), 'rrrlgvwr_settings_nonce' ) ) {
				die( esc_html__( 'Security check', 'error-log-viewer' ) );
			} else {
				...
				$this->options['to_email']   = isset( $_POST['rrrlgvwr_to_email'] ) && in_array( $_POST['rrrlgvwr_to_email'], array( 'user', 'custom' ) ) ? sanitize_text_field( wp_unslash( $_POST['rrrlgvwr_to_email'] ) ) : $this->options['to_email'];
				$this->options['email_user'] = isset( $_POST['rrrlgvwr_email_user'] ) && get_user_by( 'login', sanitize_email( wp_unslash( $_POST['rrrlgvwr_email_user'] ) ) ) ? sanitize_email( wp_unslash( $_POST['rrrlgvwr_email_user'] ) ) : '';

				if ( 1 === intval( $this->options['send_email'] ) && $this->options['file_path'] ) {
					if ( isset( $_POST['rrrlgvwr_email'] ) && is_email( sanitize_email( wp_unslash( $_POST['rrrlgvwr_email'] ) ) ) ) {
						$this->options['email']          = sanitize_email( wp_unslash( $_POST['rrrlgvwr_email'] ) );
						$this->options['frequency_send'] = isset( $_POST['rrrlgvwr_frequency_send'] ) ? absint( wp_unslash( $_POST['rrrlgvwr_frequency_send'] ) ) : '';
						$this->options['hour_day']       = isset( $_POST['rrrlgvwr_hour_day'] ) && array_key_exists( sanitize_text_field( wp_unslash( $_POST['rrrlgvwr_hour_day'] ) ), $rrrlgvwr_periods ) ? absint( wp_unslash( $_POST['rrrlgvwr_hour_day'] ) ) : 3600;
		...
		public function tab_settings() {
			...
			?>
			<input type="hidden" name="_wpnonce" value="<?php echo esc_attr( wp_create_nonce( 'rrrlgvwr_settings_nonce' ) ); ?>" />
			<?php
		}
						...
						<fieldset>
							...
							<div id="rrrlgvwr-user-email">
								<select name="rrrlgvwr_email_user">
									...
									<?php
									$rrrlgvwr_userslogin = get_users( 'blog_id=' . $GLOBALS['blog_id'] . '&role=administrator' );
						...
						<fieldset>
							...
							<select name="rrrlgvwr_hour_day">
								<?php foreach ( $rrrlgvwr_periods as $seconds => $period ) { ?>
									<option value="<?php echo esc_html( $seconds ); ?>" <?php selected( $seconds, $this->options['hour_day'] ); ?>><?php echo esc_html( $period ); ?></option>
								<?php } ?>
							</select>
						</fieldset>
					</td>
